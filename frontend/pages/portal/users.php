...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'save_user') {
            ...
            if ($name === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || !in_array($role, ['admin','humas','kasir'], true)) throw new RuntimeException('Nama, email, atau role belum valid.');
            ...
            if ($id) {
                ...
                if ($password !== '') {
                    $pdo->prepare('UPDATE portal_users SET name=?, email=?, role=?, password=? WHERE id=?')->execute([$name,$email,$role,password_hash($password,PASSWORD_DEFAULT),$id]);
                } else {
                    $pdo->prepare('UPDATE portal_users SET name=?, email=?, role=? WHERE id=?')->execute([$name,$email,$role,$id]);
                }
                if ($id === portal_user()['id']) {
                    $_SESSION['portal_user']['name'] = $name; $_SESSION['portal_user']['email'] = $email; $_SESSION['portal_user']['role'] = $role;
                }
                portal_log($pdo, 'update_user', 'Memperbarui akun: ' . $email);
            } else {
                $pdo->prepare('INSERT INTO portal_users (name,email,password,role) VALUES (?,?,?,?)')->execute([$name,$email,password_hash($password,PASSWORD_DEFAULT),$role]);
                portal_log($pdo, 'create_user', 'Membuat akun ' . $role . ': ' . $email);
            }
        ...
        portal_flash('danger', $e->getCode() === '23000' ? 'Email tersebut sudah dipakai akun lain.' : 'Data pengguna gagal disimpan.');
